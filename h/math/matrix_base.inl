...
template <class T,int size> 
T matrix_minor (const matrix<T,size>& src,int m,int n)
{
  matrix<T,size-1> tmp;
  int i,j;
...
template <class T> 
T matrix_minor (const matrix<T,2>& src,int m,int n) 
{
  return src [(m+1)&1][(n+1)&1];
} 

template <class T> 
T matrix_minor (const matrix<T,3>& src,int m,int n) 
{
...
{
  for (int i=0;i<size;i++) res [i] = dot (m [i],v);
}

template <class T,int size>
inline void matrix_mul_vec (vec<T,size>& res,const matrix<T,size+1>& m,const vec<T,size>& v)
{
  for (int i=0;i<size;i++) res [i] = dot ((vec<T,size>&)m [i],v) + m [i][size];
}
